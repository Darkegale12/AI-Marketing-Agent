...
5. HASHTAGS: (5 trending hashtags)`,
    ...
    'TikTok': `Create TikTok video script for: ${idea}. Tone: ${tone}. Target audience: ${audience}.
    
Format the response with:
1. HOOK: (first 3 seconds attention grabber)
2. SCRIPT: (60-second video script with timestamps)
3. CAPTION: (engaging caption with emojis)
4. TRENDING SOUNDS: (3 suggested trending audio tracks)`,
...
async function generateContentForPlatforms(campaignIdea, platforms, tone, targetAudience, apiKey) {
  ...
  for (const platform of platforms) {
    ...
    // Use Meme Creator for content generation (it's creative and works)
    ...
    console.log(`${result.success ? '✅' : '❌'} ${platform} content: ${result.success ? 'completed' : 'failed'}`);
  }
