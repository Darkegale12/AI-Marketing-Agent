...
const templates = {
    crypto: {
        ...
        target_audience: "Crypto traders 25-40, meme enthusiasts, active on crypto Twitter",
    ...
    nft: {
        ...
        target_audience: "NFT collectors 20-35, digital art enthusiasts, Web3 natives",
        platforms: ["X", "Instagram"],
        tone: "professional",
        budget: 45
    },
    defi: {
        ...
        target_audience: "DeFi users 28-45, yield farmers, crypto investors",
    ...
    genz: {
        ...
        target_audience: "Gen Z 18-25, social media savvy, crypto curious",
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Setup close button for content panel
    const closeContentBtn = document.getElementById('closeContentPanel');
    if (closeContentBtn) {
        closeContentBtn.addEventListener('click', closeContentPanel);
    }
});
...
function applyTemplate(templateKey) {
    ...
    document.getElementById('targetAudienceInput').value = template.target_audience || '';
    ...
    return [
        {
            ...
            description: `Analyzing target audience: ${params.target_audience || 'General audience'}`,
...
function generateMockOutput(task, campaign) {
    const outputs = {
        'Audience Research & Insights': {
            ...
            content: [
                `Primary Audience: ${campaign.target_audience || 'Crypto enthusiasts 18-35'}`,
        ...
        <div style="margin-bottom: 2rem;">
            ...
            ${campaign.target_audience ? `
                <div style="background: var(--bg-tertiary); padding: 1rem; border-radius: 8px; margin-bottom: 2rem;">
                    <p style="color: var(--text-muted); font-size: 0.875rem; margin-bottom: 0.5rem;">Target Audience</p>
...
function displayGeneratedContent(result) {
    const contentPanel = document.getElementById('contentCreatedPanel');
...
function renderTikTokContent(content) {
    const platformContentArea = document.getElementById('platformContentArea');
    const parsed = parseTikTokContent(content.mainContent);
    
    let html = '';
    
    // Hook
    if (parsed.hook) {
        html += createContentSection('🎣 Hook (First 3 Seconds)', parsed.hook);
    }
    
    // Script
    if (parsed.script) {
        html += createContentSection('🎬 Video Script', parsed.script);
    }
    ...
    // Sounds
    if (parsed.sounds) {
        html += createContentSection('🎵 Trending Sounds', parsed.sounds);
    }
...
function parseTikTokContent(content) {
    const result = { hook: '', script: '', caption: '', sounds: '' };
    
    const hookMatch = content.match(/Hook[:\s]*([\s\S]*?)(?=Script|Caption|Sounds|$)/i);
    const scriptMatch = content.match(/Script[:\s]*([\s\S]*?)(?=Hook|Caption|Sounds|$)/i);
    const captionMatch = content.match(/Caption[:\s]*([\s\S]*?)(?=Hook|Script|Sounds|$)/i);
    const soundsMatch = content.match(/(?:Trending )?Sounds?[:\s]*([\s\S]*?)(?=Hook|Script|Caption|$)/i);
    
    result.hook = hookMatch ? hookMatch[1].trim() : '';
    result.script = scriptMatch ? scriptMatch[1].trim() : content;
    result.caption = captionMatch ? captionMatch[1].trim() : '';
    result.sounds = soundsMatch ? soundsMatch[1].trim() : '';
